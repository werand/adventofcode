...
;;     write values at its current position.  A set of states, each
;;     containing rules about what to do based on the current value under
;;     the cursor.
;;
...
;; the current state says what value to write at the current position of
;; the cursor, whether to move the cursor left or right one slot, and which
;; state to use next.
;;
...
;;
;; In state A: If the current value is 0: - Write the value 1.  - Move one
;;   slot to the right.  - Continue with state B.  If the current value is
;;   1: - Write the value 0.  - Move one slot to the left.  - Continue with
;;   state B.
;;
;; In state B: If the current value is 0: - Write the value 1.  - Move one
;;   slot to the left.  - Continue with state A.  If the current value is
;;   1: - Write the value 1.  - Move one slot to the right.  - Continue
;;   with state A.
;;
...
;;
;; ... 0 0 0 [0] 0 0 ... (before any steps; about to run state A) ... 0 0 0
;; 1 [0] 0 ... (after 1 step; about to run state B) ... 0 0 0 [1] 1 0
;; ... (after 2 steps ; about to run state A) ... 0 0 [0] 0 1 0 ... (after 3
;; steps; about to run state B) ... 0 [0] 1 0 1 0 ... (after 4 steps; about
;; to run state A) ... 0 1 [1] 0 1 0 ... (after 5 steps ; about to run state
;; B) ... 0 1 1 [0] 1 0 ... (after 6 steps ; about to run state A)
;;
...
; The essence of the sample program
(def pgm {:A {:op-0 {:write      1
                     :move       :right
                     :next-state :B}
              :op-1 {:write 0
                     :move :left
...
(defn run-turing-pgm [pgm steps]
  (let [tape {:current-pos 0}]
    (loop [i 0 tape tape state :A]
      (if (< i steps)
        (let [{:keys [current-pos]} tape
              {:keys [op-0 op-1]} (get pgm state)
              op (if (= 0 (get tape current-pos 0)) op-0 op-1)
              {:keys [write move next-state]} op
              tape (assoc tape :current-pos (+ current-pos (if (= move :right) 1 -1))
                          current-pos write)]
          (recur (inc i) tape next-state))
        (reduce + (vals (dissoc tape :current-pos)))))))
