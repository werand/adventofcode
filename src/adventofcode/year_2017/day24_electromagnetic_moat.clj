...
;; input). Each port is identified by the number of pins it uses ; more pins
;; mean a stronger connection for your bridge. A 3/7 component, for
...
;;
;;     0/1 0/1--10/1 0/1--10/1--9/10 0/2 0/2--2/3 0/2--2/3--3/4
;;     0/2--2/3--3/5 0/2--2/2 0/2--2/2--2/3 0/2--2/2--2/3--3/4
;;     0/2--2/2--2/3--3/5
;;
...
(defn strength [components]
  (if (seq components)
    (reduce + 0 (flatten components))))
...
(defn build-bridges
  ...
  ([components size bridges]
   (when (seq components)
     (let [matches (filter (partial matches? size) components)]
       (if (seq matches)
         (concat [bridges]
                 (pmap #(build-bridges (remove (partial = %) components)
                                       (if (= (first %) size)
                                         (second %)
                                         (first %))
                                       (conj bridges %))
...
(defn compute-strength [bridges]
  (map strength bridges))
