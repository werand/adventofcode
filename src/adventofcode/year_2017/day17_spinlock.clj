...
;;
;;     (0), the initial state before any insertions.  0 (1): the spinlock
;;     steps forward three times (0, 0, 0), and then inserts the first
;;     value, 1, after it. 1 becomes the current position.  0 (2) 1: the
;;     spinlock steps forward three times (0, 1, 0), and then inserts the
;;     second value, 2, after it. 2 becomes the current position.  0 2 (3)
;;     1: the spinlock steps forward three times (1, 0, 2), and then
;;     inserts the third value, 3, after it. 3 becomes the current
;;     position.
;;
;; And so on:
;;
;;     0 2 (4) 3 1 0 (5) 2 4 3 1 0 5 2 4 3 (6) 1 0 5 (7) 2 4 3 6 1 0 5 7 2
;;     4 3 (8) 6 1 0 (9) 5 7 2 4 3 8 6 1
;;
...
;;
;; Your puzzle answer was 777.  --- Part Two ---
;;
...
;;
;; Your puzzle answer was 39289581.

(defn insert-into [cb pos value]
  (let [start (subvec cb 0 pos)
        end (subvec cb pos)]
    (vec (concat start [value] end))))

(defn calc-new-pos [steps size]
  (rem steps size))

(defn spinlock [steps total]
  (loop [current-pos 0
         i 1
         cb [0]]
    (println (str current-pos " " cb " "))
    (if (<= i total)
      (let [new-pos (inc (calc-new-pos (+ current-pos steps) i))]
        (recur new-pos (inc i) (insert-into cb new-pos i)))
      (nth cb (inc current-pos)))))
...
(defn calc-pos-for-1 [[size current-pos last-value-pos1]]
  (let [new-pos (inc (calc-new-pos (+ current-pos 376) size))]
    (if (= new-pos 1)
      [size new-pos size]
      [size new-pos last-value-pos1])))

(defn pos1s [n]
  (reduce #(calc-pos-for-1 [(inc %2) (second %1) (second (rest %1))]) [1 0 0] (range n)))

;(pos1s 50000000)
;=> [50000000 38032330 39289581]
