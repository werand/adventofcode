(ns adventofcode.year-2017.day15-dueling-generators
...
;;
;; Your puzzle answer was 619.  --- Part Two ---
;;
...
;;
;; Generator A looks for values that are multiples of 4.  Generator B
;; looks for values that are multiples of 8.
;;
...
(defn generator-fn [factor multiple-of prev-value]
  (let [next-value (rem (* factor prev-value) 2147483647)]
    (if (= 0 (mod next-value multiple-of))
      next-value
      (recur factor multiple-of next-value))))

;; Generators for part1
(def generator-a (partial generator-fn 16807 1))

(def generator-b (partial generator-fn 48271 1))

;; Generators for part2
(def generator-a-p2 (partial generator-fn 16807 4))

(def generator-b-p2 (partial generator-fn 48271 8))

(def bit-mask-16-bit (- (math/expt 2 16) 1))

(defn eq-last-16-bit? [a b]
  (= (bit-and bit-mask-16-bit a) (bit-and bit-mask-16-bit b)))

(defn find-matching-pairs [n a-start b-start]
  (loop [s (range n)
         pairs 0
         a-seq (rest (iterate generator-a-p2 a-start))
         b-seq (rest (iterate generator-b-p2 b-start))]
    (if (seq s)
      (let [new-pairs (if (eq-last-16-bit? (first a-seq) (first b-seq))
                        (inc pairs)
                        pairs)]
        (recur (rest s) new-pairs (rest a-seq) (rest b-seq)))
      pairs)))

;Generator A starts with 591
;Generator B starts with 393

;(find-matching-pairs (* 40 (math/expt 10 6)) 591 393)

;(find-matching-pairs (* 5 (math/expt 10 6)) 591 393)
