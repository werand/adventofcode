...
(defn show-root [node-hierarchie]
  (let [lines (s/split node-hierarchie #"\n")
        ...
        root (reduce #(disj %1 %2) nodes (flatten sub-nodes))]
    root))

#_(show-root (slurp "resources/year_2017/day07_recursice_circus"))
  ...
  ([node-hierarchie]
   (let [root (first (show-root node-hierarchie))
         ...
         node-> (reduce #(let [[node weight & sub-nodes] %2
                           ...
                           (assoc %1 node {:weight weight :sub-nodes sub-nodes})) {} words)]
     (balance-weight node-> root 0)))
  ([node-> current-node weight]
   (let [node-info (get node-> current-node)
         leaf? (nil? (:sub-nodes node-info))]
     (if leaf?
       (assoc node-info :node current-node :acc-weight (:weight node-info))
       (let [p (map #(balance-weight node-> % 0) (:sub-nodes node-info))
             _ (when (not (apply = (map :acc-weight p)))
                 (let [count (reduce #(assoc %1 (:acc-weight %2) (conj (get %1 :acc-weight []) %2)) {} p)]
                   (println (str "node: " current-node " " count))))
             sum (reduce #(+ %1 (:acc-weight %2)) 0 p)
             sum (+ (:weight node-info) sum)
             node-info (assoc node-info :node current-node :acc-weight sum)]
         node-info))
     )))

; This delivers not the correct result, manual calculation has been used to derive the puzzle answer from
; the println's above
#_(balance-weight (slurp "resources/year_2017/day07_recursice_circus"))
