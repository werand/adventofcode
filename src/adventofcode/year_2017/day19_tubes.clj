...
;;     right, up, and right, passing through B in the process.  Continue
;;     down (collecting C), right, and up (collecting D).  Finally, go all
;;     the way left through E and stopping at F.
;;
...
;;
;; Your puzzle answer was EOCZQMURF.  --- Part Two ---
;;
...
;;     3 steps right.  4 steps up.  3 steps right.  4 steps down.  3 steps
;;     right.  2 steps up.  13 steps left (including the F it stops on).
;;
...
(defn load-tube-maze [filename]
  (vec (map vec (clojure.string/split (slurp filename) #"\n"))))
...
(defn next-direction-updown [d]
  (if (= d down)
    down
    up))

(defn next-direction-leftright [d]
  (if (= d left)
    left
    right))
...
(defn is-valid-direction? [[d c]]
  (or (= c \|)
      (= c \-)
      (= c \+)
      (is-letter? c)))

(defn next-direction-at-corner [m p cd]
  (let [l [left (get-pos m (left p))]
        r [right (get-pos m (right p))]
        u [up (get-pos m (up p))]
        d [down (get-pos m (down p))]]
    (condp = cd
      left (ffirst (filter is-valid-direction? [u d]))
      right (ffirst (filter is-valid-direction? [u d]))
      up (ffirst (filter is-valid-direction? [r l]))
      down (ffirst (filter is-valid-direction? [r l]))
      (throw (IllegalStateException. "invalid direction")))))

(defrecord TubeMaze [m p d l]
  Maze
  (next-pos [this]
    (let [this (assoc this :p ((:d this) (:p this)))
          at-pos (get-pos (:m this) (:p this))
          this (if (is-letter? at-pos)
                 (assoc this :l (conj (:l this) at-pos))
                 this)]
      (cond
        (or (= at-pos \|)
            (= at-pos \-)
            (is-letter? at-pos)) this
        (= at-pos \+) (assoc this :d (next-direction-at-corner (:m this) (:p this) (:d this)))
        :default (assoc this :d :finished)))))

(defn follow-tube-path [m]
  (loop [tube-maze (TubeMaze. m [(.indexOf (first m) \|) 0] down []) steps 0]
    #_(println tube-maze)
    (if (= :finished (:d tube-maze))
      [(:l tube-maze) steps]
