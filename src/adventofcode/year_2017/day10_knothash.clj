(ns adventofcode.year-2017.day10-knothash)
...
;;  (which starts at 0), and a sequence of lengths (your puzzle
;;  input). Then, for each length:
;;
;;      Reverse the order of that length of elements in the list, starting
;;      with the element at the current position.  Move the current position
;;      forward by that length plus the skip size.  Increase the skip size
;;      by one.
;;
...
;;      current position).  The first length, 3, selects ([0] 1 2) 3 4
;;      (where parentheses indicate the sublist to be reversed).  After
;;      reversing that section (0 1 2 into 2 1 0), we get ([2] 1 0) 3 4.
...
;;      The sublist 3 4 2 1 is reversed to form 1 2 4 3: 4 3) 0 ([1] 2.  The
;;      current position moves forward by the length plus the skip size, a
;;      total of 5, causing it not to move because it wraps around: 4 3 0
;;      [1] 2. The skip size increases to 2.
;;
;;      The third length, 1, selects a sublist of a single element, and so
;;      reversing it has no effect.  The current position moves forward by
;;      the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size
;;      increases to 3.
;;
;;      The fourth length, 5, selects every element starting with the
;;      second: 4) ([3] 0 1 2. Reversing this sublist (3 0 1 2 4 into 4 2 1
;;      0 3) produces: 3) ([4] 2 1 0.  Finally, the current position moves
...
;;
;;  Your puzzle answer was 6909.  --- Part Two ---
;;
...
;;
;;      The empty string becomes a2582a3a0e66e6e86e3812dcb672a272.  AoC 2017
;;      becomes 33efeb34ea91902bb2f59c9920caa6cd.  1,2,3 becomes
...
;;
;;  Your puzzle answer was 9d5f4561367d379cfbf04f8c471c0095.

(defn wrap [size value]
  (if (< value 0)
...
(defn reverse-subvector [size v pos length]
  (loop [new-v v
         start-pos pos
         end-pos (wrap size (+ pos length -1))
         length length]
    (if (> length 0)
      (recur (assoc new-v start-pos (nth v end-pos))
             (wrap size (inc start-pos))
             (wrap size (dec end-pos))
             (dec length))
      new-v)))
...
(defn myhash
  ([lengths]
    (let [v (vec (range 0 size))]
      (myhash v lengths 0 0)))
  ([v lengths current-pos skip]
     (loop [v v
       ...
       (if (seq lengths)
         (let [length (first lengths)
               next-pos (wrap size (+ current-pos length skip))]
           (recur (reverse-subvector size v current-pos length)
                  next-pos
                  (wrap size (inc skip))
                  (rest lengths)))
         [v current-pos skip]))))
...
(defn hash-rounds [s]
  (let [[result current-pos skip] (myhash s)]
    (loop [result result
           ...
           rounds 63]
      (if (> rounds 0)
        (let [[temp-result current-pos skip] (myhash result s current-pos skip)]
...
(defn hash-string [s]
  (let [char-seq (map int (seq s))
        ...
        rounds (hash-rounds lengths)
        blocks (map #(reduce bit-xor %) (partition 16 rounds))
