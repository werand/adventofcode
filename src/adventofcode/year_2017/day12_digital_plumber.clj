...
;; pipes either directly or indirectly. The programs you identified just
;; a moment ago are all part of the same group. Now, they would like you
...
(defn get-group-map [rl]
  (let [group-map (->> (re-seq #"(\d*) <-> ([\d, ]*)" rl)
                       (map rest)
                       (map (fn [[x y]] (vector x (clojure.string/split y #", "))))
                       (reduce #(assoc %1 (first %2) (second %2)) {}))]
    group-map))


(defn determine-group
  ([group-map]
   (determine-group group-map "0" #{}))
  ([group-map current-program group]
   (if (some #{current-program} group)
     group
     (reduce #(set (concat %1 (determine-group group-map %2 (conj %1 current-program))))
             group
             (get group-map current-program)))))


(defn determine-groups [group-map]
  (loop [programs (keys group-map)
         result #{}]
    (if (seq programs)
      (recur (rest programs) (conj result (determine-group group-map (first programs) #{})))
      result)))

#_(count (determine-group
         (get-group-map "0 <-> 2
   1 <-> 1
   2 <-> 0, 3, 4
   3 <-> 2, 4
...
#_(count (determine-groups
         (get-group-map (slurp "resources/year_2017/day12_digital_plumber"))))