...
(def direction->left+right {:up [:left :right] :right [:up :down] :down [:right :left] :left [:down :up]})
...
(defn toggle-node [m pos]
  (let [new-node (if (is-infected m pos) \. \#)]
    (when (= new-node \#)
  ...
  (let [m (parse-map (slurp f))
    ...
    (loop [i (dec iterations) input input]
      ...
      (if (<= 0 i)
        ...
        [@infection-count (:direction input)]))))
